...
import { Plus, Mic, Send, X, Paperclip, Camera, Globe, FileText, BookOpen, MessageSquare, ChevronRight, Loader2, File as FileIcon, Upload, Square, Check } from "lucide-react"
...
const ChatInput = React.forwardRef<ChatInputRef, ChatInputProps>(({
  ...
  const filteredKnowledge = React.useMemo(() => {
    ...
    return knowledgeList.filter(
      (k) => !selectedKnowledgeIds.includes(k.id) &&
  ...
  const hasContent = hasText || images.length > 0 || documents.length > 0 || selectedKnowledgeIds.length > 0
  ...
  const addFilesInternal = React.useCallback((files: File[]) => {
    for (const file of files) {
      if (isImageFile(file)) {
        ...
        fileService.parseDocument(file).then((result) => {
          setDocuments((prev) =>
            prev.map((d) =>
              d.file === file
          ...
          const errorMessage = error instanceof Error ? error.message : t("chatInput.fileProcessFailedFallback")
          setDocuments((prev) =>
            prev.map((d) =>
              d.file === file
        ...
        toast.error(t("chatInput.fileFormatUnsupported", { name: file.name }))
      }
    }
  }, [])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if ((message.trim() || images.length > 0 || documents.length > 0) && onSubmit && !disabled) {
      ...
      onSubmit({
        ...
        knowledgeItems: selectedKnowledgeIds.length > 0
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (hashQuery !== null && filteredKnowledge.length > 0) {
      ...
      if (e.key === "ArrowUp") {
        e.preventDefault()
        setHashPopupIndex((prev) => (prev - 1 + filteredKnowledge.length) % filteredKnowledge.length)
        return
      }
      if (e.key === "Enter") {
        e.preventDefault()
      ...
      {isDragging && (
        <div className="absolute inset-0 z-50 flex items-center justify-center rounded-2xl border-2 border-dashed border-primary bg-primary/10 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-2 text-primary">
            ...
            <span className="text-sm font-medium">{t("chatInput.dropFile")}</span>
          </div>
        </div>
      )}
          ...
          {documents.map((doc, index) => (
            ...
            >
              ...
              <div className="flex flex-col min-w-0 pr-2">
                <span className="text-sm font-semibold truncate text-foreground">{doc.file.name}</span>
                {doc.isLoading && (
                  <span className="text-xs text-muted-foreground">{t("chatPage.processing")}</span>
                )}
                {doc.parsed && (
                ...
                {doc.error && (
                  <span className="text-xs text-destructive">{t("chatPage.failed")}</span>
                )}
              </div>
              <button
                type="button"
      ...
      {hashQuery !== null && (
        <div className="absolute bottom-full mb-2 left-0 right-0 z-50 rounded-xl border bg-popover shadow-lg overflow-hidden">
          <div className="px-3 py-1.5 border-b">
            <p className="text-xs text-muted-foreground">{t("chatInput.selectKnowledge")} <span className="font-medium text-foreground">#{hashQuery}</span></p>
          </div>
          {filteredKnowledge.length === 0 ? (
            <div className="px-3 py-3 text-center text-xs text-muted-foreground">
              {knowledgeList.length === 0 ? t("chatInput.noKnowledge") : t("chatInput.noMatch")}
            </div>
          ) : (
      ...
      <div className="relative flex items-center gap-2 rounded-2xl border bg-background p-2 shadow-sm">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" side="top" className="w-56">
            <DropdownMenuItem className="cursor-pointer gap-3" onClick={handleFileUpload}>
                      ...
                      <DropdownMenuItem
                        ...
                        onSelect={(e) => {
                          ...
                          setSelectedKnowledgeIds((prev) =>
                            selected ? prev.filter((id) => id !== kb.id) : [...prev, kb.id]
                          )
                        }}
                      >
