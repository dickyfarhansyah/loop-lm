...
function HomePage() {
  ...
  React.useEffect(() => {
    const enabledModels =
      modelsData?.data?.filter((m) => m.isEnabled !== false) ?? [];
    if (!model && enabledModels.length > 0) {
      const pendingModel = (location.state as { pendingModel?: string } | null)
        ?.pendingModel;
  ...
  const handleSubmit = async (data: {
    ...
    const { message, images, documents, knowledgeIds, knowledgeItems, notes } =
      data;
        ...
        <div className="w-full max-w-2xl space-y-8">
          <div className="flex flex-col items-center gap-4">
            <div className="flex items-center gap-3">
              <h1 className="text-3xl ">{t("homePage.greeting")}</h1>
            </div>
          </div>
