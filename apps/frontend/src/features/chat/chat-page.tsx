...
function ChatPage() {
  ...
  const [regenModelDialog, setRegenModelDialog] = React.useState<{ messageId: string } | null>(null)
  const [regenNewModel, setRegenNewModel] = React.useState("")
  const pendingRegenRef = React.useRef<{ content: string; images?: string[] } | null>(null)

  // Data hooks
  const state = location.state as LocationState | null
  const { data: chat, isLoading } = useChat(chatId ?? "")
  const { sendMessage, isStreaming, streamingContent, abortStream, optimisticMessages } = useChatStream({
    chatId: chatId ?? "",
    model,
  ...
  const [optimisticUserMessage, setOptimisticUserMessage] = React.useState<ChatMessage | null>(null)
  ...
  const existingShareUrl = isShared ? `${window.location.origin}/share/${chat.chat.share_id}` : ""

  const messages: ChatMessage[] = React.useMemo(() => {
    if (!chat?.chat?.messages) return []
    return Object.values(chat.chat.messages).sort((a, b) => a.timestamp - b.timestamp)
  }, [chat])
  ...
  const displayMessages: ChatMessage[] = React.useMemo(() => {
    ...
    for (const opt of allOptimistic) {
      const alreadyIn = base.some((m) => m.role === opt.role && m.content.includes(opt.content))
      if (!alreadyIn) {
        base = [...base, opt]
      }
    }
    ...
    return base
  }, [messages, isStreaming, streamingContent, model, optimisticUserMessage, optimisticMessages])
  ...
  // Scroll management
  const { scrollContainerRef, messagesEndRef, isUserScrolling, showScrollButton, scrollToBottom } =
    useChatScroll({
      isStreaming,
      streamingContent,
      messages,
  ...
  React.useEffect(() => {
    ...
    if (state?.pendingPromptId && !selectedPromptId) setSelectedPromptId(state.pendingPromptId)
  }, [messages, model, state?.pendingModel, selectedPromptId, state?.pendingPromptId])

  React.useEffect(() => {
    if (
      (state?.pendingMessage || state?.pendingDocuments || state?.pendingKnowledgeIds) &&
      model &&
      chatId &&
      ...
      const run = async () => {
        ...
        sendMessage(aiMessage, imageBase64, displayMessage)
      }

      run()
      navigate(location.pathname, { replace: true, state: null })
    }
  }, [state, model, chatId, isStreaming, isLoading, sendMessage, navigate, location.pathname])
    ...
    for (let i = assistantIndex - 1; i >= 0; i--) {
      if (displayMessages[i].role === "user") {
        ...
        if (regenNewModel !== model) {
          pendingRegenRef.current = { content: userMsg.content, images: userMsg.images }
  ...
  return (
    ...
    >
      {isDragging && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="flex flex-col items-center gap-3 p-8 rounded-2xl border-2 border-dashed border-primary bg-primary/10">
            ...
            <span className="text-lg font-medium text-primary">{t("chatInput.dropFile")}</span>
            <span className="text-sm text-muted-foreground">{t("chatPage.dropFileTypes")}</span>
          </div>
        </div>
      )}
      ...
      {showSearch && (
        <ChatSearchBar
          query={searchQuery}
          resultCount={filteredDisplayMessages.filter((m) => m.role !== "system").length}
          onQueryChange={setSearchQuery}
          onClose={() => setShowSearch(false)}
        />
      )}

      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden px-4">
        <div className="max-w-5xl mx-auto">
          <ChatMessageList
            ...
            onDeleteMessage={() => toast.info(t("chatPage.deleteMessagePending"))}
