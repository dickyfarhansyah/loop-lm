...
export function useChatStream({
  ...
  const sendMessage = React.useCallback(
    async (
      ...
      try {
        console.log(
          `[chat-stream] sendMessage chatId=${chatId} model=${model}`,
        );
        ...
        console.log(
          `[chat-stream] user message saved, invalidating queries...`,
        );
        ...
        const chatData = queryClient.getQueryData<{
          chat: { messages: Record<string, ChatMessage> };
        }>(["chats", chatId]);
        ...
        const chatDataForTitle = queryClient.getQueryData<{
          chat: { messages: Record<string, ChatMessage> };
        }>(["chats", chatId]);
        const userMessages = chatDataForTitle?.chat?.messages
          ? Object.values(chatDataForTitle.chat.messages).filter(
              (m) => m.role === "user",
            )
          : [];
        ...
        if (userMessages.length === 1) {
          ...
          chatService
            .generateTitle(chatId, model, messageToSave)
            .then(() => {
        ...
        if (fullContent) {
          console.log(
            `[chat-stream] saving assistant message (${fullContent.length} chars)...`,
          );
          ...
          console.log(
            `[chat-stream] assistant message saved, refetching chats list for title update...`,
          );
        ...
        if (isAborted) {
          if (partial) {
            console.log(
              `[chat-stream] aborted with partial content (${partial.length} chars), saving...`,
            );
            await chatService.addMessage(chatId, {
              role: "assistant",
